...
struct deco; /* complete in deco.h, which includes this header */

struct client {
    xcb_window_t win;
    unsigned ws; /* owning workspace index in workspaces[] */
    client_t *next, *prev;
    client_t *mru_prev, *mru_next; /* global MRU order (focus history) */

    char *name;         /* WM_NAME copy; owned by client */
    char *cls;          /* WM_CLASS class part (ICCCM) */
    bool floating;
    ...
    struct deco *deco; /* title bar wrapper, NULL when decorations off */
};
...
void raise_client(wm_t *wm, client_t *c);
